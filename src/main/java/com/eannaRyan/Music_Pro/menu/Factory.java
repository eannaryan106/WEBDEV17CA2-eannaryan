...
/**
 * Instanciate an object of a classed based on being supplied the class name as
 ...
 * casting or a run time exception will occur i.e.
 * obj.getClass().isInstance(className) returns true if is.
 *
 * WARNING this current version does not work where primitives are submitted as
 * parameters to use primitives wrap them in their respective classes i.e. int
 * use Integer, boolean use Boolean it will also be necessary to change the
 * class constructor to these wrapper classes or add a constructor
 *
 * example 1:
 *
 * The instanciation of an object with no parameters Note the name of the class
 * must contain its URL or package path.
 *
 * // get a factory object Factory f = Factory.getFactory();
 *
 * // ask it to return an object of the class given as a string parameter Object
 * obj = f.getObject( "com.oreallyoreilly.Firefish.theClass" );
 *
...
public class Factory {
	 ...
	 * provided
	 *
	 * @params className Must supply the fully qualified classname ie.
	 *         "jermuck.factory.BasicFactory" or "java.lang.Thread" for the class
	 *         you wish to instanciate
	 *
	 * @params parameters an object array of the actual parameter objects to be used
	 *         by the constructor of of the class. The parameters are all in the
	 *         form of Object classes
	 *
	 * @params parameterClassNames a string array with the corresponding class names
	 *         of the parameters contained in the parameters object array. This is
	 *         required for searching for the right constructor and casting the
	 *         objects back to their correct class type before executing the
	 *         constructor
	 *
	 **/

	public Object createObject(String className, Object[] parameters, String[] parameterClassNames)
